...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    #New event if bomb is placed by a crate (to be finished

    # neigh = np.int(np.sqrt(state_to_features(new_game_state)[0].shape[0])/2 - 0.5)
    # print('It = ', self.model[1, -1] )
    # if old_game_state:
    #     old_neigh =  state_to_features(old_game_state).reshape(2*neigh+1, 2*neigh+1)
    # if state_to_features(old_game_state)[0]
    #     crate_around = true
    # if state_to_features(new_game_state)
    # if self_action == 'BOMB' and
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    #if self.model.shape[0] == 5: self.model = np.vstack([self.model, np.zeros(226) ])
