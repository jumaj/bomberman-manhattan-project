...
def setup(self):
    ...
    if self.train and not os.path.isfile("tau_" + str(self.tau) + "_gamma_" + str(gamma) + "_temp_" + str(self.T) + "_epsilon_" + str(self.epsilon) + ".pt"):
        self.logger.info("Setting up model from scratch.")
        weights = np.array([0, 0, 1, 0, 0, 0])#.25, .25, .25, .25, 0, 0])#np.random.rand(len(ACTIONS))
        self.model = weights / weights.sum()
        self.mod = 0
    else:
        self.logger.info("Loading model from saved state.")
        self.mod = 1
        with open("tau_" + str(self.tau) + "_gamma_" + str(gamma) + "_temp_" + str(self.T) + "_epsilon_" + str(self.epsilon) + ".pt", "rb") as filem:
            self.model = pickle.load(filem)
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    neigh = 1
    agent_neigh = np.array(arena_ext[(16+x-neigh):(17+x+neigh),(16+y-neigh):(17+y+neigh)]).reshape(9)
    agent_neigh_close = []
    for t in [1,3,5,7]:
    ...
    channels = [np.sign(targetcoin.reshape(2).T), agent_neigh_close]
